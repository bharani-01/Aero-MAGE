...
export default function DashboardLayout({ role: propRole, children }: DashboardLayoutProps) {
  ...
  // Resolve role headers & links (Clean main navigation links)
  let roleDisplayName = '';
  ...
  const userName = user?.displayName || user?.display_name || user?.name || user?.username || 'User';
  const userEmail = user?.email || user?.email_address || 'Registered User';
    ...
    <div className="bg-surface font-body-md text-on-surface min-h-screen flex flex-col selection:bg-primary-fixed selection:text-on-primary-fixed pb-20 lg:pb-8">
      ...
      <nav className="w-full bg-white/95 backdrop-blur-md border-b border-outline-variant shadow-sm sticky top-0 z-50">
        <div className="flex justify-between items-center px-4 md:px-8 py-3 max-w-7xl mx-auto w-full">
          ...
          {/* Desktop Web View Navigation Links */}
          ...
          {/* Top Right Action & Profile Avatar Dropdown */}
          <div className="flex items-center gap-3">
            {/* Join Live Quiz Desktop Button */}
            <button
            ...
            <div className="relative" ref={profileMenuRef}>
              ...
              {/* Expanded Profile Menu Dropdown */}
              {showProfileMenu && (
      ...
      {/* Mobile Bottom Navigation Bar with Dynamic Role-Isolated Links */}
      <nav className="lg:hidden fixed bottom-0 left-0 w-full z-50 bg-white/95 backdrop-blur-md border-t border-outline-variant shadow-lg py-1">
        <div className="grid grid-cols-5 items-center max-w-lg mx-auto w-full px-1">
          ...
          {/* Floating Center Join Button (Symmetric 50% Center Grid Placement) */}
          ...
          {/* 3rd role-specific link (e.g. Quiz Library) */}
          {links[2] ? (
            <button
      ...
      {/* Global Join Live Quiz / Classroom Modal */}
      {showJoinModal && (
