...
export default function FacultyLayout({ children }: FacultyLayoutProps) {
  ...
  const userName = user?.displayName || user?.display_name || user?.name || user?.username || 'Faculty';
  const userEmail = user?.email || user?.email_address || 'Faculty Member';
  const userAvatar = user?.photoURL || user?.avatar_url || user?.profile_image || user?.avatar;
  const userInitial = userName.charAt(0).toUpperCase();

  return (
    <MaintenanceOverlay userRole="faculty">
      ...
      {/* Top Navigation Bar - Faculty Dedicated */}
      <nav className="w-full bg-white/95 backdrop-blur-md border-b border-outline-variant shadow-sm sticky top-0 z-50">
        <div className="flex justify-between items-center px-4 md:px-8 py-3 max-w-7xl mx-auto w-full">
          ...
          {/* Logo & Role Badge */}
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate('/faculty/quizzes')}>
      ...
      {/* Mobile Bottom Navigation Bar - Faculty Dedicated */}
