...
export default function Marketplace() {
  ...
  // Rate Modal State
  const [rateItem, setRateItem] = useState<MarketplaceItem | null>(null);
  ...
  const handleSubmitRating = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem('accessToken');
    if (!token) {
      alert('Please log in to rate marketplace quizzes.');
      return;
    }
      ...
      <div className="flex flex-col gap-6 max-w-7xl mx-auto w-full">
        ...
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-xs font-bold text-slate-400 animate-pulse">Loading marketplace listings…</p>
          </div>
        ) : items.length === 0 ? (
          <div className="bg-white border border-outline-variant rounded-3xl p-12 text-center flex flex-col items-center gap-3 shadow-sm">
            ...
            <h3 className="text-lg font-bold text-slate-800">No Marketplace Quizzes Found</h3>
            <p className="text-xs text-slate-500 max-w-sm">
              Try adjusting your search query, category, or difficulty filter. Be the first to publish a quiz to the marketplace!
            </p>
          </div>
        ) : (
