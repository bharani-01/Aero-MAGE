...
function gradeAnswer(question, submittedAnswer, elapsedSeconds) {
    ...
    if (qType === 'multiple_choice' || qType === 'true_false' || qType === 'audio_mcq' || qType === 'video_mcq' || qType === 'image_choice') {
        ...
        const correct = String(question.correct_answer || '').split(',').map((s) => s.trim()).filter(Boolean).sort().join(',');
        ...
        const correct = String(question.correct_answer || '').split('|').map((s) => s.trim()).join('|');
        isCorrect = submitted === correct;
    }
    else if (qType === 'match_following') {
        // Submitted as "Left1:Right1,Left2:Right2" sorted; correct_answer same format
        const normalise = (s) => s.split(',').map(pair => pair.trim()).sort().join(',');
...
export const launchLiveSession = async (req, res) => {
...
export const getLiveSessionState = async (req, res) => {
...
export const handleHostAction = async (req, res) => {
    const { sessionId } = req.params;
...
export const submitParticipantAnswer = async (req, res) => {
    const { sessionId } = req.params;
